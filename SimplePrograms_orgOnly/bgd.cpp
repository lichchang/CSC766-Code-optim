...
double example5() {
    ...
    std::fill(w, w + D, 0);  // Initialize w array to 0

    for (int k = 0; k < NOiteration; k++) {
        std::fill(d, d + D, 0);  // Reset d array to 0 at the start of each iteration

        // Compute dot products once per iteration, store results
