...
double example1() {
  ...
  // Precompute the contributions of y and s to reduce redundant multiplications
  for (int j = 0; j < m; j++) {
    for (int k = 0; k < n; k++) {
      for (int l = 0; l < n; l++) {
        temp[j][k] += y[l][j] * s[j][k];
      }
    }
  }
  
  // Perform the final multiplication and accumulation
  for (int i = 0; i < m; i++) {
    for (int k = 0; k < n; k++) {
      for (int j = 0; j < m; j++) {
        r[i][k] += x[i][j] * temp[j][k];
      }
    }
  }
